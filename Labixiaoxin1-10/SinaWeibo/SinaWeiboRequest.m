...
- (void)appendUTF8Body:(NSMutableData *)body
...
{
    id result = [JsonUtils JSONObjectWithData:data];
...
+ (NSString *)getParamValueFromUrl:(NSString*)url
    ...
    for (NSString* key in [params keyEnumerator]) {
        if (([[params objectForKey:key] isKindOfClass:[UIImage class]])
            ...
            if ([httpMethod isEqualToString:@"GET"]) {
                MSLog(@"can not use GET to upload a file");
            }
            continue;
        }
