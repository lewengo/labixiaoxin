...
//  Roosher
//
//  Created by shenjianguo on 10-10-12.
//  Copyright 2010 Roosher. All rights reserved.
//

#import "RunLoopOperation.h"
...
@interface RunLoopOperation ()

// read/write versions of public properties

@property (assign, readwrite) RunLoopOperationState    state;
@property (copy,   readwrite) NSError *                error;          

@end

@implementation RunLoopOperation

- (id)init {
    self = [super init];
    if (self != nil) {
        assert(_state == kRunLoopOperationStateInited);
    }
    return self;
}
...
- (void)setState:(RunLoopOperationState)newState {
    // any thread

    @synchronized (self) {
        RunLoopOperationState  oldState;
...
//        assert( (newState != kRunLoopOperationStateFinished) || self.isActualRunLoopThread );
        ...
        oldState = _state;
        if ( (newState == kRunLoopOperationStateExecuting) || (oldState == kRunLoopOperationStateExecuting) ) {
            [self willChangeValueForKey:@"isExecuting"];
        }
        if (newState == kRunLoopOperationStateFinished) {
...
//    assert(self.state == kRunLoopOperationStateExecuting);
...
- (void)cancelOnRunLoopThread {
    ...
    // We know that a) state was kRunLoopOperationStateExecuting when we were 
    ...
    if (self.state == kRunLoopOperationStateExecuting) {
        [self finishWithError:[NSError errorWithDomain:NSCocoaErrorDomain code:NSUserCancelledError userInfo:nil]];
    }
}
...
- (void)finishWithError:(NSError *)error {
    ...
    self.state = kRunLoopOperationStateFinished;
}
...
- (BOOL)isExecuting {
    // any thread
    return self.state == kRunLoopOperationStateExecuting;
}
 
- (BOOL)isFinished {
    // any thread
    return self.state == kRunLoopOperationStateFinished;
}

- (void)start {
    // any thread

    assert(self.state == kRunLoopOperationStateInited);
    ...
    self.state = kRunLoopOperationStateExecuting;
    ...
    else [self startOnRunLoopThread];
}
...
- (void)cancel {
    ...
    @synchronized (self) {
        ...
        runCancelOnRunLoopThread = ! oldValue && self.state == kRunLoopOperationStateExecuting;
    }
